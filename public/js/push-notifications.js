...
export async function initPushNotifications(orderId, button) {
  ...
  if (Notification.permission === "granted") {
    button.hidden = true;
    activerToken(orderId).catch((err) => console.error("Échec de l'enregistrement du jeton :", err));
    return;
  }
...
async function activerToken(orderId) {
  ...
  await enregistrerTokenNotification({ commandeId: orderId, token });
  ...
  onMessage(messaging, (payload) => {
    const titre = payload.notification?.title || "Makitti";
